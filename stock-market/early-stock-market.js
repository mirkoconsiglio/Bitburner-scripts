...
export async function main(ns) {
	...
	const short = ns.getPlayer().bitNodeN === 8 || ns.getOwnedSourceFiles().some(s => s.n === 8 && s.lvl > 1);
	...
	while (true) {
		if (symLastPrice['FSIG'] === ns.stock.getPrice('FSIG')) continue;

		for (const sym of ns.stock.getSymbols()) {
			const current = ns.stock.getPrice(sym);
			symChanges[sym].push(current / symLastPrice[sym]);
			symLastPrice[sym] = current;
			if (symChanges[sym].length > samplingLength) symChanges[sym] = symChanges[sym].slice(symChanges[sym].length - samplingLength);
		}
		...
		for (const sym of prioritizedSymbols) {
			...
			if (longShares <= 0 && shortShares <= 0 && ns.stock.getPrice(sym) < 30000) continue;
			...
			if (longShares > 0) {
				...
				if (state < 0) {
					...
					if (sellPrice > 0) ns.print(`SOLD (long) ${sym}. Profit: ${ns.nFormat(profit, '0.000a')}`);
				} else ns.print(`${sym} (${ratio}): ${ns.nFormat(profit + cost, '0.000a')} / ${ns.nFormat(profit, '0.000a')} (${(profit / cost * 100).toFixed(2)}%)`);
				...
				if (state > 0) {
					...
					if (sellPrice > 0) ns.print(`SOLD (short) ${sym}. Profit: ${ns.nFormat(profit, '0.000a')}`);
				} else ns.print(`${sym} (${ratio}): ${ns.nFormat(profit + cost, '0.000a')} / ${ns.nFormat(profit, '0.000a')} (${(profit / cost * 100).toFixed(2)}%)`);
			} else {
				const money = ns.getServerMoneyAvailable('home');
				if (state > 0) {
					const sharesToBuy = Math.min(10000, ns.stock.getMaxShares(sym), Math.floor((money - commission) / askPrice));
					if (ns.stock.buy(sym, sharesToBuy) > 0) ns.print(`BOUGHT (long) ${sym}.`);
				} else if (state < 0 && short) {
					const sharesToBuy = Math.min(10000, ns.stock.getMaxShares(sym), Math.floor((money - commission) / bidPrice));
					if (ns.stock.short(sym, sharesToBuy) > 0) ns.print(`BOUGHT (short) ${sym}.`);
				}
			}
		}
		await ns.sleep(1000);
